...
def vitSmall(patchSize=16, **kwargs):
    return VisionTransformerIBot(patch_size=patchSize, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, **kwargs)


def vitBase(patchSize=16, **kwargs):
    return VisionTransformerIBot(patch_size=patchSize, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, **kwargs)
...
class ExtratorIBot:
    ...
    def carregarModelo(self):
        print(f"[IBot] carregando {self.tipoModelo}...")
        ...
        if os.path.exists(caminhoPesos):
            ...
            # o checkpoint do ibot salva com prefixo 'backbone.' ou 'module.'
            pesosLimpos = {k.replace('backbone.', '').replace('module.', ''): v for k, v in pesos.items()}
            self.model.load_state_dict(pesosLimpos, strict=False)
            print(f"[IBot] pesos carregados!")
        else:
            raise FileNotFoundError(f"pesos nao encontrados: {caminhoPesos}")

        self.model = self.model.to(self.device)
        self.model.eval()
        self.registrarHooks()
        print(f"[IBot] ok | device={self.device} | dim={self.dimEmbed}")
    ...
    def extrairFeatures(self, x, aplicarGAP=True):
        # usa media dos patches (ignora CLS token)
        self.features = {}
        with torch.no_grad():
            _ = self.model(x)
        ...
        for i in range(self.numBlocos):
            ...
            if chave in self.features:
                ...
                if aplicarGAP:
                    feat = feat[:, 1:].mean(dim=1) if feat.dim() == 3 else feat.mean(dim=[2, 3])
                resultado[f'block{i}'] = feat.cpu()
        return resultado

    def extrairFeaturesComCLS(self, x):
        # usa o CLS token (posicao 0) de cada bloco
        self.features = {}
        with torch.no_grad():
            _ = self.model(x)
        ...
        for i in range(self.numBlocos):
            ...
            if chave in self.features:
                feat = self.features[chave]
                resultado[f'block{i}'] = (feat[:, 0] if feat.dim() == 3 else feat).cpu()
        return resultado
