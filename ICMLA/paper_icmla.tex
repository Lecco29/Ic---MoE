...
%
% Melhorias aplicadas em relação à versão SMC:
%   1. Título atualizado (Attribute-Aware Image Retrieval...)
%   2. Análise qualitativa do roteador + figura fig_router_behavior.pdf
%   3. Justificativa do protocolo de retrieval (training set as gallery)
%   4. LMFCN incluído como referência de comportamento por camada apenas
%   5. "generalizes across architectures" → "consistently observed across
%      the evaluated architectures"
%   6. Figura: distribuição de α e β para cor e textura
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[conference]{IEEEtran}
\IEEEoverridecommandlockouts
% \IEEEoverridecommandlockouts é necessário para o \thanks na primeira página
...
\usepackage{float}
\usepackage{multirow}
\usepackage{epstopdf}    % converte .eps para .pdf automaticamente no pdflatex
...
\title{Attribute-Aware Image Retrieval via Adaptive Layer Fusion
...
\author{%
\IEEEauthorblockN{Arlete T. Beuren, Vin\'{i}cius C. Ribeiro,
                  Leonardo J. R. Pinto}
\IEEEauthorblockA{Universidade Tecnol\'{o}gica Federal do Paran\'{a}
                  (UTFPR)\\
...
\IEEEauthorblockA{UEPG / Pontif\'{i}cia Universidade Cat\'{o}lica
                  do Paran\'{a} (PUCPR)\\
                  Brazil\\
...
a pattern consistently observed across the evaluated architectures,
from classical CNNs to Vision Transformers and State Space Models.
...
deep layer to Block~3~(256d) (Fig.~\ref{fig_vgg}).
...
Stage~2~(384d) (Fig.~\ref{fig_mamba}).
...
(Fig.~\ref{fig_lmfcn}).
LMFCN is included as a reference architecture for layer-wise behavior
only, and is therefore evaluated under Experiments~1 and~2 only, since
integrating a trainable MoE router into its framework would require
non-trivial architectural modifications.
...
For retrieval evaluation, we use the training set as the retrieval gallery
to ensure disjoint query and database sets, avoiding trivial matches.
...
Fig.~\ref{fig_depth} illustrates the full depth-accuracy curves,
...
For each test image used as a query, all training images in the gallery
are ranked by Euclidean distance; metrics are computed over the
...
previously observed in classical CNNs is consistently observed across the
evaluated architectures, including Vision Transformers and State Space
Models.
...
The trainable router adapts to image content: images whose discriminability
is driven by color induce higher early-layer weights~($\alpha$), while
images whose discriminability is driven by texture induce higher
deep-layer weights~($\beta = 1-\alpha$).

Fig.~\ref{fig_router} illustrates this routing behavior using the iBOT
backbone.
For the color-optimized model, the early-layer weight~$\alpha$ (median
$\approx$0.55) is consistently higher than for the texture-optimized
model (median $\approx$0.44), confirming that the router learns to
allocate representational capacity according to the dominant visual
attribute — without any explicit routing supervision.
The asymmetry is particularly informative for iBOT: early blocks~(e.g.,
Block~2) naturally capture low-level chromatic information, so images
whose class identity is driven by color consistently push~$\alpha$ above
0.5; conversely, images where texture is the discriminant attribute shift
the router toward Block~9 representations~($\beta > 0.5$).
Qualitatively, images with uniform, saturated chromatic regions (e.g.,
solid-color garments) tend to drive~$\alpha$ toward its upper range,
whereas images with repeated structural patterns (e.g., striped or
checkered fabrics) drive~$\beta$ toward its upper range.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.46\textwidth]{fig_router_behavior.pdf}}
\caption{Distribution of early-layer router weights~$\alpha$ for
         color-trained and texture-trained MoE models (iBOT, fold~1).
         The color-optimized model consistently assigns higher weight to
         the early layer (median $\alpha \approx 0.55$), while the
         texture-optimized model shifts weight toward the deep layer
         (median $\alpha \approx 0.44$, i.e., $\beta \approx 0.56$).
...
Experiments~1 and~2 confirm that the layer-wise sensitivity pattern is
consistently observed across the evaluated architectures — from classical
CNNs to Vision Transformers and State Space Models — and that simple
concatenation fusion is not effective for attribute-specific tasks.
