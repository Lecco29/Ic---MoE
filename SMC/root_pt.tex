...
\title{\LARGE \bf
Sensibilidade por Camada e Fusão Adaptativa com Mixture of Experts
em Modelos Profundos para Classificação de Cor e Textura
}
...
Este trabalho aborda o desafio de extrair efetivamente o conteúdo de
...
classificação de cor, enquanto camadas profundas se destacam no
reconhecimento de textura, padrão que se generaliza de CNNs clássicas
a Vision Transformers e Modelos de Espaço de Estados.
O \emph{Experimento~2} mostra que a concatenação simples de características
de camadas rasa e profunda geralmente não supera representações
especializadas de camada única.
O \emph{Experimento~3} introduz um modelo MoE treinável que, por meio de
um roteador aprendido, pondera adaptativamente as contribuições da camada
rasa (sensível a cor) e da camada profunda (sensível a textura) por imagem.
...
únicos e têm aplicações além do comércio eletrônico, incluindo imagens
médicas, sistemas de informação geográfica e vigilância~\cite{b12}.
Um desafio central consiste em extrair com precisão atributos visuais
relevantes de imagens, especialmente quando a consulta do usuário é guiada
por propriedades específicas como cor, textura ou sua combinação
...
Descritores clássicos de cor incluem histogramas de cor e momentos de cor,
que capturam propriedades estatísticas como média, variância e assimetria
das distribuições de cor~\cite{b24}.
Para representação de textura, métodos como filtros de Gabor e Padrões
Binários Locais~(LBP) têm sido amplamente utilizados~\cite{b25}.

Com o advento do aprendizado profundo, Baloian et al.~\cite{b2}
demonstraram que diferentes camadas de CNNs exibem sensibilidade específica
a atributos, com camadas iniciais sendo mais responsivas à informação de cor
e camadas mais profundas capturando padrões de textura.
Contudo, o comportamento de representações específicas de atributos em
arquiteturas modernas — Vision Transformers e Modelos de Espaço de Estados
— permanece pouco explorado, e estratégias de fusão adaptativa entre camadas
especializadas ainda são escassas na literatura.

Neste trabalho, avaliamos e comparamos arquiteturas clássicas e modernas de
aprendizado profundo para classificação de imagens por cor e textura.
...
por imagens de moda incluindo chapéus, vestidos, camisas, meias, bolsas e
lenços.
É dividido em dois subconjuntos:
\begin{itemize}
...
\begin{itemize}
  ...
  \item Benchmark comparativo de VGG-16, ResNet-50, iBOT, VMamba e LMFCN
        para classificação de imagens por cor e textura;
  ...
  \item Proposta e avaliação de um modelo MoE que realiza fusão adaptativa
        de características rasa e profunda, superando tanto concatenação
        quanto representações de camada única na maioria das configurações.
\end{itemize}
...
objetos foi amplamente explorado na literatura.
...
ImageNet~\cite{b3}, com \emph{Global Average Pooling}~(GAP) aplicado a
cada bloco residual e k-vizinhos mais próximos~(kNN) utilizado para
classificação com 5 repetições de holdout estratificado 70/30.

A natureza hierárquica das representações em redes profundas foi investigada
desde os trabalhos seminais de Zeiler e Fergus~\cite{bzeiler}, que mostraram
visualmente que camadas iniciais de CNNs respondem a bordas e cores enquanto
camadas profundas codificam padrões estruturais complexos.
Análises análogas para Vision Transformers foram conduzidas por
Kazami et al.~\cite{bkazami}, revelando que blocos iniciais do ViT preservam
informação espacial de baixo nível enquanto blocos finais capturam semântica
global.
Em contraste com esses trabalhos, que realizam análise qualitativa ou de
visualização, o presente estudo conduz uma avaliação quantitativa sistemática
de sensibilidade por camada a atributos visuais específicos~(cor e textura),
estende a análise a arquiteturas de Modelos de Espaço de Estados~(VMamba)
ainda não contempladas na literatura de análise por camada, e propõe um
mecanismo de fusão adaptativa treinável fundamentado nessas observações.
...
redes neurais para aprender características discriminativas, e Gan
et al.~\cite{b11}, que propuseram técnicas de generalização para domínio
multi-fonte em aprendizado de atributos.
...
Lin et al.~\cite{b15} introduziram matrizes de co-ocorrência de
cor~(CCM), diferença entre pixels~(DBPSP) e agrupamento baseado em
histograma de cor~(CHKM).
...
Estratégias incluem fusão antecipada~(concatenação de características),
fusão tardia~(nível de decisão) e fusão aprendida com mecanismos de
atenção~\cite{b26,b27}.
No entanto, a eficácia dessas abordagens para tarefas de atributo específico
ainda é insuficientemente estudada.

Avanços recentes desafiaram a dominância de modelos baseados em Transformer.
A arquitetura Mamba~\cite{b16} é um Modelo de Espaço de Estados
Estruturado~(SSM) com complexidade linear no comprimento da sequência, e o
framework iBOT~\cite{b17} introduziu modelagem de imagem mascarada com
tokenizador online para aprendizado autossupervisionado de representações
visuais.
O framework LMFCN~\cite{b28} treina Redes Totalmente Convolucionais cujas
representações são otimizadas para classificação de grande margem com SVMs.
...
aprende, por imagem, quanto ponderar a representação da camada
rasa~(cor) versus a camada profunda~(textura).

% -----------------------------------------------------------------------
\section{ARQUITETURAS BACKBONE AVALIADAS}
...
residual via conexões de atalho baseadas em identidade, mitigando a
degradação do gradiente e permitindo treinamento estável em grande
profundidade.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{resnet50.png}}
...
profunda ao Bloco~3~(256d) (Figura~\ref{fig_vgg}).

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{VGG16.jpg}}
...
O VMamba~\cite{b16} é um Modelo de Espaço de Estados Estruturado~(SSM) que
processa sequências em tempo linear por meio de um mecanismo de varredura
seletiva.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{Vmamba.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{Ibot.jpg}}
...
cujas representações aprendidas são otimizadas para classificação de grande
margem com SVM.
Avaliamos arquiteturas TCNN~(3 a 6 camadas) treinadas do zero e variantes
da ResNet-18 pré-treinada com as camadas finais removidas
(Figura~\ref{fig_lmfcn}).
Como a LMFCN depende de um classificador SVM externo fortemente acoplado ao
objetivo de treinamento da FCN, integrar um roteador MoE treinável ao seu
framework exigiria modificações arquiteturais não triviais; a LMFCN é
portanto avaliada apenas nos Experimentos~1 e~2.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{CNNSV4.eps}}
...
Os dados foram divididos em 70\% para treinamento e 30\% para teste, com
5 repetições de holdout estratificado 70/30.
Seguindo o protocolo estabelecido em~\cite{b2} e amplamente adotado na
literatura de CBIR baseado em atributos, os vetores de características são
avaliados com um classificador k-vizinhos mais próximos~(kNN) com $k=5$
...
CNNs usam GAP para produzir um vetor de características por camada; iBOT
usa o token CLS; VMamba aplica GAP ao mapa de características de cada
estágio.
Para LMFCN, avaliamos variantes TCNN~(3 a 6 camadas) e ResNet-18 com 2 ou
4 blocos residuais.

\subsection{Fusão por Concatenação Rasa--Profunda}

Para cada backbone, as características da melhor camada para cor~(E) e da
melhor camada para textura~(D) são concatenadas:
...
$\beta$ para a camada rasa e a camada profunda, respectivamente.

\textbf{Arquitetura.} Para cada backbone, dois projetores $h_E$ e $h_D$
— compostos por uma camada linear, LayerNorm e ReLU — mapeiam
características de dimensões heterogêneas para um espaço comum de dimensão
$d = 256$.
Um roteador~(Router), baseado em MLP de duas camadas com Softmax, recebe
a concatenação $[h_E(f_E) ; h_D(f_D)]$ e produz os pesos $\alpha$ e
$\beta$ com $\alpha + \beta = 1$.
O vetor final é:
\begin{equation}
...
\emph{cross-entropy loss}; na avaliação final, somente os embeddings~$z$
são utilizados com kNN.

\textbf{Protocolo de treinamento.}
Otimizador AdamW, $lr = 10^{-4}$, \emph{weight decay} $= 10^{-5}$,
CosineAnnealingLR com $T_{\max} = 30$ épocas, \emph{batch size} 32,
aumento de dados básico~(recorte aleatório 224$\times$224 e flip
horizontal) somente no treinamento.
Avaliação final: kNN com $k = 5$ e StandardScaler nos embeddings~$z$,
sob o mesmo protocolo de holdout 70/30 com 5 repetições.

A Tabela~\ref{tab_moe_config} resume as seleções de camada rasa e profunda
por backbone no modelo MoE.
...
A primeira parte~(Seções~V-A e~V-B) analisa o comportamento por camada
dos embeddings de cada backbone em relação a cor e textura, utilizando a
acurácia do kNN como proxy da discriminabilidade dos embeddings.
O objetivo aqui não é otimizar a classificação, mas caracterizar quais
camadas melhor capturam cada atributo visual.
A segunda parte~(Seções~V-C a~V-E) avalia as estratégias de fusão e,
por fim, avalia os mesmos embeddings sob o protocolo CBIR com
mAP@10, R@1 e R@5.

\subsection{Sensibilidade por Camada}

A Tabela~\ref{tab_best_layer} resume a melhor camada por backbone para
classificação de cor e textura.
Em todas as arquiteturas, camadas rasas alcançam consistentemente maior
acurácia de cor, enquanto camadas profundas são mais eficazes para
textura — confirmando e estendendo o padrão originalmente reportado para
a ResNet-50 em~\cite{b2}.
Na VGG-16, a Camada~1 obteve pico de 95,13\% em cor, enquanto a
Camada~4 atingiu 94,73\% em textura.
A ResNet-50 seguiu a mesma tendência: Bloco Residual~2 para
cor~(95,87\%) e Bloco~3 para textura~(93,27\%).
O iBOT apresentou o gradiente mais acentuado entre seus 12 blocos
transformer — a acurácia de cor caiu de 94,53\% no Bloco~0 para 62,80\%
no Bloco~11, enquanto a de textura subiu para 97,20\% no Bloco~9, o
valor mais alto entre todas as arquiteturas avaliadas.
O VMamba exibiu versão mais comprimida do mesmo padrão em seus quatro
estágios, com o Estágio~1 como melhor para cor~(94,41\%) e o Estágio~2
para textura~(95,76\%).

\begin{table}[htbp]
\caption{Melhor camada por backbone para classificação de cor e textura
         (\%). Negrito: maior valor por coluna de atributo.}
\begin{center}
\begin{tabular}{c|c|c|c|c|c|c}
\hline
\textbf{Backbone} & \textbf{Mel. Cor} & \textbf{Cor} & \textbf{Std}
                  & \textbf{Mel. Tex.} & \textbf{Tex.} & \textbf{Std} \\
\hline
VGG-16    & Cam. 1 & 95,13          & 0,81 & Cam. 4  & 94,73          & 0,74 \\
ResNet-50 & BR 2   & \textbf{95,87} & 0,50 & BR 3    & 93,27          & 0,25 \\
iBOT      & Bl. 2  & \textbf{95,87} & 1,05 & Bl. 9   & \textbf{97,20} & 0,54 \\
...
A Figura~\ref{fig_depth} ilustra as curvas completas de acurácia por
profundidade, mostrando o padrão inverso entre cor e textura em cada
backbone.
...
A LMFCN com SVM alcançou 96,87\%$\pm$0,9 em textura~(ResNet-18 com 2
blocos) e 97,73\%$\pm$0,95 em cor~(TCNN3).
...
A Tabela~\ref{tab5} apresenta a configuração de fusão por backbone, e as
...
Para cor, a fusão degradou consistentemente a acurácia comparada ao uso da
camada especializada em cor~(E), com o iBOT apresentando a maior
degradação~($-$11,80\%).
...
Comparado à extração de camada única e à fusão por concatenação, o MoE
supera ambos em quase todas as combinações de backbone e atributo.
...
avaliação dos embeddings sob o protocolo CBIR: para cada imagem de consulta
do conjunto de teste, todas as imagens de treino são ranqueadas por
distância euclidiana e as métricas são calculadas sobre os $K\!=\!10$
mais próximos.
mAP@10 é a precisão média nos 10 vizinhos mais próximos; R@1 e R@5
indicam, respectivamente, a fração de consultas em que o resultado correto
aparece na primeira e entre as cinco primeiras posições.
...
A exceção do iBOT em textura confirma o mesmo padrão observado na acurácia:
o Bloco~9 gera representações de textura tão discriminativas~(mAP@10 =
96,40\%) que o roteador não consegue superá-las.
O ganho do MoE sobre a concatenação é expressivo para iBOT cor~(+22,90~pp
em mAP@10), confirmando que a fusão adaptativa resolve o problema de
degradação observado no Experimento~2.

% -----------------------------------------------------------------------
\section{DISCUSSÃO}

Os resultados experimentais confirmam que o padrão de sensibilidade por
camada observado em CNNs clássicas estende-se às arquiteturas modernas
baseadas em Vision Transformers e Modelos de Espaço de Estados.
...
de textura — consistente com os achados fundacionais de Zeiler e
Fergus~\cite{bzeiler} para CNNs e com as análises do ViT de Kazami
et al.~\cite{bkazami}.
Este trabalho estende esses achados quantitativamente a tarefas de
classificação por atributo específico e ao VMamba, uma arquitetura de
Modelo de Espaço de Estados ainda não contemplada na literatura de análise
por camada.
...
A dimensionalidade uniforme dos blocos do iBOT~(384d) contrasta com as
dimensões crescentes em CNNs e VMamba, mas o iBOT alcança discriminação de
textura superior, indicando que a qualidade da representação — e não a
dimensionalidade — é o fator determinante.
...
aprender dinamicamente os pesos de contribuição por imagem.
...
configurações avaliadas, e superior à camada única em 6 das 8
configurações.

A exceção do iBOT em textura~($-$1,73~pp sobre o Exp.~1) é explicada pelo
fato de que o Bloco~9 do iBOT já fornece representações de textura
excepcionalmente ricas~(97,20\%); o roteador não consegue melhorar o que já
está próximo do limite do conjunto.
Para a VGG-16 em cor, a assimetria dimensional entre a camada rasa
pequena~(64d) e a camada profunda~(256d) pode limitar a capacidade do
projetor~$h_E$.

Em termos práticos, o MoE oferece uma representação unificada para sistemas
de recuperação de atributos visuais, eliminando a necessidade de selecionar
manualmente a camada mais adequada para cada tipo de consulta.
...
Espaço de Estados, e que a concatenação simples não é uma estratégia eficaz
de fusão para tarefas de atributo específico.
O Experimento~3 introduz o MoE como uma solução que aprende adaptativamente
os pesos das representações rasa~(cor) e profunda~(textura), superando tanto
a extração de camada única quanto a fusão por concatenação em 6 das 8
...
especialistas, mecanismos de atenção cruzada entre camadas, e extensão para
tarefas de recuperação em larga escala.
